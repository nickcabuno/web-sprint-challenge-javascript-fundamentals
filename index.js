...
const zooAnimals = [
 ...
 function animalNames(arr){
  const fullNames = [];
    arr.forEach(item => fullNames.push(`name: ${item.animal_name}, scientific: ${item.scientific_name}`));
    return fullNames
  }
 ...
 function lowerCaseNames(arr){
  const lowNames = arr.map(item => (item.animal_name.toLowerCase()));
    return lowNames
}
 ...
 function USApop(arr){
  const avg = arr.reduce(function(accumulator, item){ 
  return accumulator + item.population;
  },0);
  return Math.round((avg / arr.length)* 100) / 10;
}
